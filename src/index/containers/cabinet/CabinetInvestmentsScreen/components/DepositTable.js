...
export default function WithdrawalTable({
  ...
  let headings = [
    ...
    <UI.TableColumn align="right">
      {utils.getLang("cabinet_profileScreen_invested")}
    </UI.TableColumn>
  ];
  ...
  if (fromPartners) {
    headings.push(
      <UI.TableColumn>
        {utils.getLang("cabinet_partners_userProfit")}
      </UI.TableColumn>,
      <UI.TableColumn>
        {utils.getLang("cabinet_partners_agentProfit")}
      </UI.TableColumn>
    );
  } else {
    headings.push(
  ...
  if (adaptive) {
    headings = [
      ...
      <UI.TableColumn
        align="right"
        sub={<>{utils.getLang("global_days")}/ID</>}
      >
        {utils.getLang("cabinet_investmentsScreen_profit")}
      </UI.TableColumn>
    ];
  ...
  const rows = deposits.map((item, i) => {
    ...
    if (progress >= 1) {
      icon = (
        ...
        >
          <path
            d="M0 12C0 18.6274 5.37258 24 12 24C18.6274 24 24 18.6274 24 12C24 5.37258 18.6274 0 12 0C5.37258 0 0 5.37258 0 12ZM21.6 12C21.6 17.3019 17.3019 21.6 12 21.6C6.69807 21.6 2.4 17.3019 2.4 12C2.4 6.69807 6.69807 2.4 12 2.4C17.3019 2.4 21.6 6.69807 21.6 12Z"
            fill="#A1A1A1"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M16.7071 9.29289C16.3166 8.90237 15.6834 8.90237 15.2929 9.29289L11 13.5858L8.70711 11.2929C8.31658 10.9024 7.68342 10.9024 7.29289 11.2929C6.90237 11.6834 6.90237 12.3166 7.29289 12.7071L9.58579 15C10.3668 15.781 11.6332 15.7811 12.4142 15L16.7071 10.7071C17.0976 10.3166 17.0976 9.68342 16.7071 9.29289Z"
            fill="#A1A1A1"
          />
        </svg>
      );
    ...
    if (adaptive) {
      return (
        <UI.TableCell
          key={item.id}
          onClick={() => {
            actions.openModal("deposit_info", {
              depositId: item.id
            });
          }}
        >
          ...
          <UI.TableColumn
            align="left"
            sub={`${item.percent}% ${item.description} ${
              item.type !== "pool" ? utils.ucfirst(item.type) : ""
            }`}
          >
          ...
          <UI.TableColumn
            sub={
              <>
                {item.passed_days} / {item.days}{" "}
                {utils.getLang("global_days", true)} /{" "}
                {utils.formatTableId(deposits.length - i)}
              </>
            }
            align="right"
          >
            {utils.formatDouble(item.profit)} {item.currency.toUpperCase()}
          </UI.TableColumn>
        </UI.TableCell>
      );
    }
    ...
    return (
      <UI.TableCell key={item.id} onClick={onClick}>
        ...
        {item.type === "pool" ? (
          ...
          <UI.TableColumn sub={item.description}>
            {item.plan_percent}%
          </UI.TableColumn>
        )}
        <UI.TableColumn align="right">
        ...
        <UI.TableColumn
          sub={
            <>
              {item.passed_days} / {item.days} {utils.getLang("global_days")}
            </>
          }
          align={fromPartners ? "left" : "right"}
        >
          {utils.formatDouble(item.profit)} {item.currency.toUpperCase()}
        </UI.TableColumn>
        {fromPartners && (
          <UI.TableColumn>
            {utils.formatDouble(item.agent_profit)}{" "}
