...
export default class InviteAgent extends React.Component {
  ...
  render() {
    return (
      <div className="InviteAgent Content_box">
        <div className="InviteAgent__title">{utils.getLang('cabinet_agentInvite_title')}</div>
        <div className="InviteAgent__form">
          ...
          >{utils.getLang('global_invite')}</UI.Button>
        </div>
      </div>
    )
  }
  ...
  __buttonDidPress = () => {
    ...
    profileActions.inviteAgent(login).then(() => {
