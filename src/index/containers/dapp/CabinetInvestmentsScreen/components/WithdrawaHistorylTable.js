...
export default function WithdrawalTable({
    ...
    if (adaptive) {
      return (
        <UI.TableCell key={i}>
          <UI.TableColumn
            style={{ width: "50%" }}
            sub={item.currency.toUpperCase()}
          >
            <span
              className={"Investment__withdrawal_table__status " + item.status}
            >
          ...
          <UI.TableColumn
            sub={
              ...
              " / " +
              utils.formatTableId(withdrawalsTotalCount - i)
            }
          >
  ...
  return (
    <UI.Table
      headings={headings}
      className="Investment__withdrawal_table"
      header={utils.getLang("cabinet_investmentsWithdrawalHistory")}
    >
      {rows}
    </UI.Table>
  );
