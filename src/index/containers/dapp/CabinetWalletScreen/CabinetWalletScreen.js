...
import { web3SetStatus, web3SetData } from 'actions/cabinet/web3';
...
import SwapFormAdaptive from 'src/index/containers/cabinet/DappCabinet/components/SwapFormAdaptive/SwapFormAdaptive';
...
class CabinetWalletScreen extends React.PureComponent {
  componentDidMount() {
    ...
    this._mounted = true;
    const { web3SetStatus, web3SetData, web3Status, fetchWalletPage } =
      this.props;

    if (
      !web3Status.isRequested &&
      !web3Status.isWalletsLoaded &&
      !web3Status.isBalancesLoaded
    ) {
      fetchWalletPage();
      // Request for wallets
  ...
  handleLoadMore = (balanceId, isCrypto, isSwap) => {
    this.props.walletFetchHistoryMore(
      buildOptions(balanceId, isCrypto, isSwap)
    );
  };

  getBalanceId = () => {
    const { route, balance, oldWallets } = this.props;
    const { name, params } = route;
    ...
    const currentToken = Object.keys(cryptoBalances).find(
  ...
  render() {
    const { status, history, route, cardReservation, isAdaptive, rates } =
      this.props;
