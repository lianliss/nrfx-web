...
const BotInfo = props => {
  const { bot } = props;
  const [primaryCurrency, secondaryCurrency] = bot.symbol
    .toLowerCase()
    .split("/");
  ...
  const handleToggleBot = () => {
    props.setStatusBot(
      props.bot.id,
      bot.status === "activated" ? "deactivated" : "activated"
    );
  };
      ...
      <UI.ContentBox className="Bot__info">
        <div className="Bot__info__column">
          ...
          <div className="Bot__info__uptime">
            ...
            {activated ? (
              <div>{utils.dateFormat(bot.start_date, null).fromNow()}</div>
            ) : (
              <div className="Bot__info__uptime__status">{bot.status}</div>
            )}
          </div>
        </div>
        <div className="Bot__info__column">
          <UI.List
            items={[
              {
                label: "ROE",
                value: <UI.NumberFormat number={bot.roe} color percent />
              },
              {
                label: "Кол-во контрактов",
                value: <div>{bot.trade_amount} </div>
              },
              { label: "Интервал", value: bot.time_frame },
              { label: "Позиция", value: bot.position },
              {
                label: "Контрактов в позиции",
                value: (
                  <div>
                    {bot.position_amount} / {bot.max_trade_amount}
                  </div>
                )
              },
              {
