...
import DexSettingsModal from '../DexSettingsModal/DexSettingsModal';
import SVG from 'utils/svg-wrap';
import CabinetBlock from 'src/index/components/cabinet/CabinetBlock/CabinetBlock';
import DexSwapInput from '../DexSwapInput/DexSwapInput';
...
class DexSwapOld extends React.PureComponent {
  ...
  state = {
    ...
    transactions: window.localStorage.getItem('DexSwapTransactions')
  ...
  async requireChain(id = 56) {
    const { chainId, isConnected, switchToChain, findTokenBySymbol, tokens } =
      this.context;
  ...
  componentDidMount() {
    ...
    this.balanceUpdateInterval = setInterval(
      this.updateExchangeTokenBalance.bind(this),
  ...
  async updateLiquidity() {
    ...
    try {
      ...
      if (
        ...
        this.pairs = pairs;
        this.updateTrade();
        this.liquidityUpdateTimeout = setTimeout(
          () => this.updateLiquidity(),
  ...
  updateTrade(
    ...
    const amount = !_.isNull(_amount)
      ? _amount
      : isExactIn
      ? _.get(this.state, 'amount0', 1)
      : _.get(this.state, 'amount1', 1);
    if (!token0 || !token1 || !amount || !this.pairs || !this.pairs.length)
      return;
  ...
  swapPair() {
    ...
    const trade = this.updateTrade(
      ...
      this.state.exactIndex ? this.state.amount1 : this.state.amount0
    );
    this.setState({
      pair: [this.state.pair[1], this.state.pair[0]],
  ...
  async updateExchangeTokenBalance() {
    try {
      ...
      this.setState((state) => {
        if (
          symbol === _.get(state, 'pair[0].symbol') &&
  ...
  async approve() {
    ...
    try {
      await this.tokenContract.approve(
        routerAddress,
        amount > maxApprove ? amount : maxApprove
      );
      this.setState({
  ...
  render() {
    ...
    const { isConnected, connectWallet, tokens, numberToFraction } =
      this.context;
    const switchTabs = [
    ...
    const isExactIn = !exactIndex;
    const executionPrice = !!this.trade
      ? significant(this.trade.executionPrice)
      : '0';
    const inputAmount = !!this.trade
      ? this.trade.inputAmount.asFraction
      : new Fraction(JSBI.BigInt(0));
    const outputAmount = !!this.trade
      ? this.trade.outputAmount.asFraction
      : new Fraction(JSBI.BigInt(0));

    const slippageFraction = numberToFraction(slippageTolerance).divide(100);
    const slippageAmount = !this.trade
      ? new Fraction(JSBI.BigInt(0))
      : isExactIn
    ...
    const priceImpact = !!this.trade
      ? this.trade.priceImpact.asFraction
      : new Fraction(JSBI.BigInt(0));
    ...
    const route = path.map((token, index) => {
      return (token.symbol === 'WBNB' && index && index !== path.length - 1) ||
    ...
    let button = (
      <Button type="lightBlue" onClick={() => openModal('connect_to_wallet')}>
    ...
    return (
      <div className="DexSwap">
        <CabinetBlock>
          <div className="DexSwap__form">
            ...
            {!_.isNull(selectToken) && (
              <CabinetModal onClose={this.closeTokenSelect}>
                <TokenSelect
                  onChange={(value) => {
        ...
        {!!route && !!route.length && (
          <DexRoute tokens={tokens} route={route} />
        )}
