...
class TranslatorModal extends React.Component {
  ...
  __load() {
    Promise.all(['en', this.props.translatorLangCode].map(code => loadLang(code, false))).then(() => {
      const currentValue = getLang(this.props.langKey, true, this.props.translatorLangCode);
      this.setState({
        status: '',
        currentValue,
        value: currentValue
  ...
  __handleChangeLanguage = () => {
    openModal('language', { byTranslator: true });
  };
  ...
  renderContent() {
    ...
    if (state.status) {
      return <LoadingStatus inline status={state.status} onRetry={() => this.__load()} />
    }
    ...
    return (
      <>
        ...
        <div className="Translation__title Translation__lang" onClick={this.__handleChangeLanguage}>
          <div>{lang.title} translation</div>
          <span><SVG src={require(`asset/site/lang-flags/${this.props.translatorLangCode}.svg`)}/></span>
        </div>
        <UI.Input
