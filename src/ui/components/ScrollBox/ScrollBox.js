...
export default class ScrollBox extends React.Component {
  ...
  handleScroll = e => {
    this.setState({
      scrollPosition: e.target.scrollTop / ( this.state.contentHeight - this.state.containerHeight )
    });
  };
  ...
  handleChange = e => {
    ...
    const buttonHeightPx = state.containerHeight / state.contentHeight * state.containerHeight;
    ...
    if (scrollPosition >= 0 && scrollPosition <= 1) {
      ...
      scrollPosition = scrollPosition > 1 ? 1 : scrollPosition < 0 ? 0 : scrollPosition;
      this.setState({ scrollPosition });
    }

    this.refs.scroll.scroll(0, scrollPosition * (state.contentHeight - state.containerHeight));
  };
  ...
  render() {
    ...
    const buttonHeightPx = state.containerHeight / state.contentHeight * state.containerHeight;
    const scrollPositionPx = state.scrollPosition * (state.containerHeight - buttonHeightPx);

    return (
      <div style={props.style} ref="container" className={cn("ScrollBox", props.className, { init: state.scrollWidth === null })}>
        <div className="ScrollBox__contentWrapper" ref="scroll" style={{ marginRight: -state.scrollWidth + 'px' }}>
          <div className="ScrollBox__content" ref="content">{props.children}</div>
        </div>
        <div className={cn("ScrollBox__bar", {
          hidden: state.contentHeight === state.containerHeight
        })}>
          <div
            ref="button"
            style={{height: buttonHeightPx + 'px', top: scrollPositionPx + 'px'}}
            onMouseDown={this.handleMouseDown}
            onMouseUp={this.handleMouseUp}
            className={cn("ScrollBox__bar__button", { active: state.buttonActive })}
          />
        </div>
      </div>
    )
