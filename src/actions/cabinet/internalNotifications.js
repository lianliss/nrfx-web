...
export function load() {
  return (dispatch, getStore) => {
    api.call(apiSchema.Notification.InternalGet).then(notifications => {
      dispatch({
      ...
      toastsActions.toastPush("Error load notifications", "error")(dispatch, getStore);
    });
  }
}
...
export function push(notification) {
  return (dispatch, getStore) => {
    dispatch({ type: actionTypes.INTERNAL_NOTIFICATION_PUSH, payload: notification});
  };
}
