...
function push(message, type) {
  ...
  const hideToast = (id) => {
    setTimeout(() => {
      const currentToast = store.getState().toasts.items.find(toast => toast.id === id);
      if (currentToast.hide) {
        store.dispatch({ type: actionTypes.TOASTS_HIDE, id })
        setTimeout(() => store.dispatch({ type: actionTypes.TOASTS_DROP, id }), 2000);
      } else {
        hideToast(id);
      }
    }, 3000);
  ...
  store.dispatch({ type: actionTypes.TOASTS_PUSH, payload: { type, message, id, hide: true }});
  hideToast(id);
}
