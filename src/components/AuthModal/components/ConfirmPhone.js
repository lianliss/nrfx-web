...
const codes = countries.map(country => ({
  title: (
    <span className="AuthModal__Phone__dropdown" key={country.code}>
      <img
        src={`https://cdnjs.cloudflare.com/ajax/libs/flag-icon-css/3.2.1/flags/4x3/${country.code.toLowerCase()}.svg`}
        alt={country.name}
      />
      {country.dial_code}
    </span>
...
function ConfirmPhone({ changeStep, params }) {
  ...
  return (
    <div className="AuthModal__Phone">
      <UI.ModalHeader>
        {utils.getLang("site__authModalConfirmNumber")}
      </UI.ModalHeader>
      ...
      <div className="AuthModal__footer">
        ...
        >
          {utils.getLang("site__authModalSendCode")}
        </UI.Button>
      </div>
    </div>
  );
