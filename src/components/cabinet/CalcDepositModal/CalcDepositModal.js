...
const CalcDepositModal = class extends React.Component {
  ...
  __handleChangeDay = (dayId, options) => {
    this.setState({ days: this.state.days.map( (day, id) => {
      if (dayId === id) {
        return { ...day, ...options}
      } return day;
  ...
  __handleAddDay = () => {
    this.setState({ days: [
      ...this.state.days, { dayNumber: null, amount: null }
    ]})
  };
    ...
    return (
      <div className="CalcDepositModal__description">
        ...
        <div className="CalcDepositModal__description__value">
          {utils.formatDouble(profit)} {this.state.currency.toUpperCase()} ({utils.formatDouble(percent, 2)}%)
        </div>
      </div>
    )
  };
      ...
      <UI.Modal className="CalcDepositModal__wrapper" isOpen={true} onClose={this.props.onClose}>
        ...
        <div className="CalcDepositModal">
          ...
          <div className="CalcDepositModal__row">
            ...
            <div className="CalcDepositModal__column">
              {this.__renderProfit(utils.getLang('cabinet_IncomeWithoutConclusions'), this.state.amount / 100 * this.state.planPercent, this.state.planPercent)}
            </div>
          </div>

          { !!this.state.days.length && <div className="CalcDepositModal__delimiter" /> }
              ...
              return (
                <div className="CalcDepositModal__day">
                  <div className="CalcDepositModal__row ">
                    <div className="CalcDepositModal__column">
                      <UI.Input
                        onTextChange={dayNumber => this.__handleChangeDay(dayId, {dayNumber})}
                        placeholder={utils.getLang('global_day')}
                        indicatorWidth={50}
                        disabled={disabled}
                        value={day.dayNumber}
                        error={(day.amount && !day.dayNumber) || (day.dayNumber && dayId && day.dayNumber <= this.state.days[dayId - 1].dayNumber)}
                        indicator={day.dayNumber && utils.getLang('global_day')}/>
                    </div>
                    <div className="CalcDepositModal__column">
                      {this.state.daysResult[dayId] && this.__renderProfit(
                        `${utils.getLang('cabinet_incomeDay_prefix')} ${day.dayNumber} ${utils.getLang('cabinet_incomeDay_postfix')}:`,
                  ...
                  <div className="CalcDepositModal__row">
                    <div className="CalcDepositModal__column amount">
                      <UI.Input
                        placeholder={utils.getLang('global_withdraw')}
                        indicatorWidth={40}
                    ...
                    <div className="CalcDepositModal__column">
                      {this.state.daysResult[dayId] && this.__renderProfit(
                        utils.getLang('global_decreaseInPercentageOfWithdrawal'),
